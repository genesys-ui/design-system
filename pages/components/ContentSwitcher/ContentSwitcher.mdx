...
Preview the ContentSwitcher component with the React live demo. For detailed code usage
...
<section className="mrow">
  <div className="mcol-12 mcol-sm-6">
    <CardReact url="https://devoinc.github.io/genesys-ui/?path=/docs/components-core-navigation-contentswitcher--docs" />
  </div>
  <div className="mcol-12 mcol-sm-6">
...
<DesignTokensList
  tokens={getCmpTokensJson('contentSwitcher')}
  intro={
    <p style={{ margin: '1.6rem 0' }}>
      There are {getCmpTokensJson('contentSwitcher').length} ContentSwitcher
      design tokens. The units in rem are not based in browser, but in base 10,
      so <code>1rem = 10px</code>.
    </p>
  }
/>
