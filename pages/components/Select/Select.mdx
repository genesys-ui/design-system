...
<section className="mrow">
  <div className="mcol-12 mcol-sm-6">
    <CardReact url="https://devoinc.github.io/genesys-ui/?path=/docs/components-form-select--docs" />
  </div>
  <div className="mcol-12 mcol-sm-6">
...
<DesignTokensList
  tokens={getCmpTokensJson('select')}
  intro={
    <p style={{ margin: '1.6rem 0' }}>
      There are {getCmpTokensJson('select').length} Select design tokens. The
